...
from estimating_uncertainties_enso.compute_lib.stat_lib import stat_smooth_triangle
...
# Main
# ---------------------------------------------------------------------------------------------------------------------#
def z03_creating_distributions(
                    ...
                    for dat in list(values[dia][dur][pro][exp].keys()):
                        if dat == fig_selected_model:
                            ...
                            if dur == data_epoch_lengths[0]:
                                arr_y = []
                                for ts in tim_values[dia_tim][pro][exp][dat][:3]:
                                    arr_y.append(stat_smooth_triangle(ts.to_numpy(), 25))
    ...
    for dia in list(data_to_plot.keys()):
        ...
        fig_name = __file__.split("/")[-1].split(".")[0] + "_" + str(dia) + str(fig_name_add) + "_03"
